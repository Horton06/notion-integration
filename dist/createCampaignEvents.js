...
async function createCampaignEvents(campaignPageId) {
    // Retrieve the campaign page from Notion
    const campaignPage = await notion.pages.retrieve({
        page_id: campaignPageId,
    });
    // Extract campaign details.
    // Change property names to match exactly your database configuration.
    const campaignName = campaignPage.properties['Campaign Name'].title[0]?.plain_text;
